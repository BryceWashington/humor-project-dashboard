...
function CaptionExamplesContent() {
  ...
  const fetchData = async () => {
    setLoading(true)
    let query = supabase.from('caption_examples').select('*, images(id, url), creator:profiles!created_by_user_id(first_name, last_name, email), modifier:profiles!modified_by_user_id(first_name, last_name, email)', { count: 'exact' })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      await (supabase.from('caption_examples') as any).insert({
        ...payload,
